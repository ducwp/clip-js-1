      ...
      {textElements.map((clip, index) => (
        <div key={clip.id} className="bg-green-500">
          <div
            ...
            className={`absolute border top-2 h-8 rounded text-white text-sm flex items-center justify-center cursor-pointer ${
              activeElement === "text" &&
              textElements[activeElementIndex]?.id === clip.id
                ? "bg-[#3F3F46] border-blue-500"
                : "bg-[#27272A] border-gray-500 border-opacity-50"
            }`}
            style={{
              left: `${clip.positionStart * timelineZoom}px`,
              width: `${
                (clip.positionEnd - clip.positionStart) * timelineZoom
              }px`,
              zIndex: clip.zIndex,
            }}>
            {/* <MoveableTimeline /> */}
            <Type
              className="h-4 w-4 min-w-4 mr-2 shrink-0"
              height={30}
              width={30}
            />
          ...
          <Moveable
            ...
            renderDirections={
              activeElement === "text" &&
              textElements[activeElementIndex] &&
              textElements[activeElementIndex].id === clip.id
                ? ["w", "e"]
                : []
            }
